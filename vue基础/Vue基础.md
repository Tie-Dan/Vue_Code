...
- 模块：模块化侧重的功能的封装，主要是针对Javascript代码，隔离、组织复制的javascript代码，将它封装成一个个具有特定功能的的模块。
- 组件：组件化更多关注的UI部分，页面的每个部件，比如头部，内容区，弹出框甚至确认按钮都可以成为一个组件，每个组件有独立的HTML、css、js代码。
...
**单页应用**(single page web application，**SPA**)，是在一个页面完成所有的业务功能，浏览器一开始会加载必需的HTML、CSS和JavaScript，单页面的页面跳转，仅刷新局部资源。

![](D:\BaiduNetdiskDownload\itcast\01\02-note\assets\sigle.jpg)

### 什么是多页应用

**多页面应用**（MultiPage Application，MPA）由多个独立页面构成，多页面跳转刷新所有资源，每个页面的资源(js、css等)需重新加载

![](D:\BaiduNetdiskDownload\itcast\01\02-note\assets\mupage.jpg)

### 单页应用与多页应用的区别

![](D:\BaiduNetdiskDownload\itcast\01\02-note\assets\Snipaste_2019-06-12_11-39-20.png)
